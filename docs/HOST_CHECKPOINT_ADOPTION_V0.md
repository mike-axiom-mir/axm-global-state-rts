...
Adoption is explicit and records both sides of the replacement in browser evidence:
...
The adoption endpoint, expected-revision rule, deterministic replay, and replacement path do not branch on human versus machine controller kind. Controller identity remains part of the host seat binding and admission evidence, but the same physical checkpoint is adopted through the same reconciliation mechanism.

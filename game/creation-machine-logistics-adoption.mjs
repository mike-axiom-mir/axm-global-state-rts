...
async function fetchCandidate(plan, { includeBytes = true } = {}) {
  ...
  if (
    ...
    throw new Error(`${plan.sourceAsset} prepared receipt failed the logistics identity contract`);
  }
...
function resolveLiveConstructionTarget(bridge, seatId, plan, buildingInstanceId = null) {
  ...
  if (buildingInstanceId) {
    ...
    if (!exact) throw new Error(`${buildingInstanceId} is not a live ${plan.gameplayDefinitionId} construction target on ${seatId}`);
    return exact;
  }
...
export async function inspectCreationMachineLogisticsSet() {
  const candidates = [];
  for (const entry of creationMachineLogisticsTrialPlan()) candidates.push(await fetchCandidate(entry, { includeBytes: false }));
...
if (typeof window !== 'undefined' && !Object.prototype.hasOwnProperty.call(window, '__AXM_CREATION_MACHINE_LOGISTICS_SET__')) {
  Object.defineProperty(window, '__AXM_CREATION_MACHINE_LOGISTICS_SET__', {
    value: Object.freeze({
      inspect: inspectCreationMachineLogisticsSet,
      adoptAsset: adoptCreationMachineLogisticsAsset,
      plan: creationMachineLogisticsTrialPlan
    }),
    configurable: false
  });
}

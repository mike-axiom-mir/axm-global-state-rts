...
function requirePlanEntry(stableAssetId) {
  ...
  const entry = creationMachineResourceTrialPlan().find(candidate => candidate.stableAssetId === key);
  if (!entry) throw new Error(`no Creation Machine resource candidate registered for ${key || 'empty stable asset id'}`);
  return entry;
}
...
export async function inspectCreationMachineResourceSet() {
  const candidates = [];
  for (const entry of creationMachineResourceTrialPlan()) {

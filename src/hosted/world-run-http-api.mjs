...
export const WORLD_RUN_HTTP_API_SCHEMA = 'axm.global-state-rts.world-run-http-api/v0.10';
        ...
        return response(200, {
          ...
          runLifecycle: Object.freeze({
            ...
            truthBoundary: persistence.durableRolloverPreparation?.enabled
              ? 'next-drop-run-start plus bounded global-control food-policy unit-training vehicle-licensing and terminal-close mutations are host-authoritative and replayable;terminal runs are archived;archive-bound rollover can be prepared and then executed into one new durable generation carrying only banked score/run history,with run-store-first account reconciliation across restart but no multi-host or atomic-database claim'
              : persistence.terminalRunArchive?.enabled
                ? 'next-drop-run-start plus bounded global-control food-policy unit-training vehicle-licensing and terminal-close mutations are host-authoritative and replayable;terminal runs are idempotently archived while safe generation rollover still requires durable run-start storage'
                : persistence.enabled
                  ? 'next-drop-run-start plus bounded global-control food-policy unit-training vehicle-licensing and terminal-close mutations are host-authoritative and replayable from durable evidence;read-only deterministic checkpoints compare that evidence across restart'
                  : 'next-drop-run-start global-control food-policy unit-training vehicle-licensing and run-close commands are host-authoritative in-process but active progression and durable checkpoint evidence remain unavailable without durable run-start storage'
          })
        });
      }

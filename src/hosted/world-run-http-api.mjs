...
export class WorldRunHttpApiService {
  ...
  handle({ method = 'GET', pathname, searchParams = null, body = {} } = {}) {
    ...
    try {
      ...
      if (verb === 'POST' && route === '/api/world/run/construct-vehicle') {
        if (this.writeMode !== 'dev') return response(403, { error: 'world writes disabled', writeMode: this.writeMode });
        const constructionAuthority = authorityWithMethod(this.runAuthority, 'constructVehicle');
        if (!constructionAuthority) return response(503, { error: 'host vehicle-construction authority unavailable' });
        const result = constructionAuthority.constructVehicle({
          participantId: body.participantId,
          runId: body.runId,
          mutationId: body.mutationId,

...
export const PLAYER_PROGRESSION_SCHEMA = 'axm.global-state-rts.player-progression/v0.1';
export const CIVILIZATION_RUN_SCHEMA = 'axm.global-state-rts.civilization-run/v0.1';
...
function frozenItemCounts(map) {
  return Object.freeze(Object.fromEntries([...map.entries()].sort((a, b) => a[0].localeCompare(b[0]))));
}
...
export class CivilizationRun {
  constructor({
    ...
    if (!blueprintLedger?.has || !blueprintLedger?.unlock || !blueprintLedger?.endRun) {
      throw new TypeError('blueprintLedger required');
    }
  ...
  #assertOpen() {
    if (this.closed) throw new Error('civilization run is already closed');
  }

  unlockMatchBlueprint(blueprintId, eventId) {
    this.#assertOpen();
    const result = this.blueprints.unlock(blueprintId, {
      source: 'match-only',
      runId: this.runId,
      eventId
    });
  ...
  setFoodPolicy(policyId) {
    this.#assertOpen();
    const result = this.food.setPolicy(policyId);
    this.revision += 1;
    return result;
  }
  ...
  trainUnit(unitId, roleId, options = {}) {
    this.#assertOpen();
    const result = this.manpower.train(unitId, roleId, {
  ...
  licenseUnit(unitId, licenseId, options = {}) {
    this.#assertOpen();
    const result = this.manpower.license(unitId, licenseId, {
      stockpile: this.stockpile,
      ...options
    });
    if (result.accepted) this.revision += 1;
  ...
  assignVehicle(unitId, options = {}) {
    this.#assertOpen();
    const result = this.manpower.assignVehicle(unitId, options);
    if (result.accepted) this.revision += 1;
    return result;
  }
  ...
  advanceFood(deltaSeconds, options = {}) {
    this.#assertOpen();
    const result = this.food.advance(deltaSeconds, {
      population: this.manpower.units.size,
      stockpile: this.stockpile,
      ...options
    });
    this.revision += 1;
  ...
  recordEnemyMaterialDestroyed(materialValue) {
    ...
    if (result.food > 0) {
      this.stockpile.credit({ food: result.food }, {
        reason: 'combat-destruction-food',
        eventId: `combat-food:${this.economy.revision}`
      });
    }
    this.revision += 1;
    return result;
  ...
  recordGlobalControlPercent(percent) {
    this.#assertOpen();
    const result = this.economy.recordGlobalControlPercent(percent);
    this.revision += 1;
    return result;
  }

  recordTerritoryLedger(territoryLedger, ownerId = this.civilizationId) {
    this.#assertOpen();
    const result = this.economy.recordTerritoryLedger(territoryLedger, ownerId);
    this.revision += 1;
    return result;
  }
  ...
  close() {
    ...
    return Object.freeze({
      ...
      finalGold: economy.finalGold,
      peakGlobalControlPercent: economy.peakGlobalControlPercent,
      destroyedEnemyMaterial: economy.destroyedEnemyMaterial,
  ...
  snapshot() {
    return Object.freeze({
      schema: CIVILIZATION_RUN_SCHEMA,
      runId: this.runId,
      civilizationId: this.civilizationId,
      revision: this.revision,
      closed: this.closed,
      stockpile: this.stockpile.snapshot(),
      food: this.food.snapshot(),
      manpower: this.manpower.snapshot(),
      blueprints: this.blueprints.snapshot({ runId: this.runId }),
      startingItems: frozenItemCounts(this.startingItems),
...
export class PlayerProgression {
  ...
  accrueDropCaches(nowMs) {
    const result = this.dropCache.accrue(nowMs);
  ...
  openDropCaches(count = 1) {
    ...
    for (const crate of result.opened) {
      ...
      for (const itemId of crate.startingItems) {
        this.dropReserveItems.set(itemId, (this.dropReserveItems.get(itemId) || 0) + 1);
      }
    }
    this.revision += 1;
    return Object.freeze({
      ...result,
      reserve: this.dropReserveSnapshot()
    });
  }

  unlockResearchBlueprint(blueprintId, eventId) {
    if (this.activeRun?.closed === false) {
      // Research may occur during a run, but its provenance remains permanent rather than silently match-scoped.
    }
    const result = this.blueprints.unlock(blueprintId, { source: 'research', eventId });
    this.revision += 1;
    return result;
  }

  unlockQuestBlueprint(blueprintId, eventId) {
    const result = this.blueprints.unlock(blueprintId, { source: 'quest', eventId });
    this.revision += 1;
    return result;
  }

  beginRun(runId, {
    crewCount = this.baseCrewCount,
    foodPolicy = 'normal',
    ...
    const runResources = addResources(
      addResources(this.baseStartingResources, this.dropReserveResources),
      extraStartingResources
    );
    const runItems = [];
    for (const [itemId, count] of this.dropReserveItems) {
      for (let i = 0; i < count; i++) runItems.push(itemId);
    }
    runItems.push(...startingItems.map(String));

    this.dropReserveResources = {};
    this.dropReserveItems.clear();
    this.activeRun = new CivilizationRun({
      runId,
      civilizationId: this.playerId,
      blueprintLedger: this.blueprints,
      startingResources: runResources,
      startingItems: runItems,
      crewCount,
      foodPolicy
    });
    this.revision += 1;
  ...
  closeActiveRun() {
    ...
    this.runHistory.push(Object.freeze({
      runId: result.runId,
      finalGold: result.finalGold,
      peakGlobalControlPercent: result.peakGlobalControlPercent,
      destroyedEnemyMaterial: result.destroyedEnemyMaterial
    }));
    this.activeRun = null;
    this.revision += 1;
  ...
  dropReserveSnapshot() {
    return Object.freeze({
      resources: Object.freeze({ ...this.dropReserveResources }),
      items: frozenItemCounts(this.dropReserveItems)
    });
  }

  snapshot() {
    return Object.freeze({
      schema: PLAYER_PROGRESSION_SCHEMA,
      playerId: this.playerId,
      revision: this.revision,
      bankedGold: this.bankedGold,
      blueprints: this.blueprints.snapshot({ runId: this.activeRun?.runId || null }),
      dropCache: this.dropCache.snapshot(),
      dropReserve: this.dropReserveSnapshot(),
      activeRun: this.activeRun?.snapshot() || null,
      runHistory: Object.freeze([...this.runHistory])
    });
  }
}

export function createPlayerProgression(options = {}) {
  return new PlayerProgression(options);
}

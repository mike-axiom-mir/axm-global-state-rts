...
for (const sourceAsset of CANDIDATES) {
  test(`explicit ${sourceAsset} alternate replaces only the real seat-1 light-tower fixture in four-seat LOCAL RTS`, async ({ page }) => {
    ...
    const result = await page.evaluate(async ({ selectedSourceAsset }) => {
      ...
      const helper = await import('./creation-machine-defense-adoption.mjs');
      return helper.adoptCreationMachineDefenseAsset({
        seatId: 'seat-1',
    ...
    expect(result.status).toBe('RUNTIME_IMPORTED_SINGLE_DEFENSE_ALTERNATE_NOT_VISUALLY_ACCEPTED');
    expect(result.candidateId).toBe(`defense-light-tower-a:${sourceAsset}`);
    expect(result.stableAssetId).toBe('defense-light-tower-a');
    expect(result.sourceAsset).toBe(sourceAsset);
    expect(result.candidateRole).toBe('explicit-alternate-static-visual-candidate');
    expect(result.gameplayTarget).toBe('starter-region:defense-light-tower-a');
    expect(result.constructionDefinitionId).toBe('building:light-tower');
    ...
    expect(retained[0].assets[0].assetId).toBe('defense-light-tower-a');

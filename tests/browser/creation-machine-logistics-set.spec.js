...
for (const sourceAsset of CANDIDATES) {
  test(`explicit ${sourceAsset} trial attaches only to a real constructed LOCAL Storage Depot and stays seat-local`, async ({ page }) => {
    ...
    expect(adopted.status).toBe('RUNTIME_IMPORTED_SINGLE_LOGISTICS_ALTERNATE_NOT_VISUALLY_ACCEPTED');
    expect(adopted.candidateId).toBe(`building-storage-depot-a:${sourceAsset}`);
    expect(adopted.stableAssetId).toBe('building-storage-depot-a');
    expect(adopted.sourceAsset).toBe(sourceAsset);
    expect(adopted.candidateRole).toBe('explicit-alternate-static-visual-candidate');

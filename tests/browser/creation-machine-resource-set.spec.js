...
const SOURCE_TRIALS = Object.freeze([
  Object.freeze({ sourceAsset: 'shallow-mine-entrance', candidateRole: 'primary-static-candidate', screenshot: 'global-state-rts-creation-machine-shallow-mine-4seat.png' }),
  Object.freeze({ sourceAsset: 'deep-mine-head', candidateRole: 'explicit-alternate-static-candidate', screenshot: 'global-state-rts-creation-machine-deep-mine-head-4seat.png' })
]);

for (const trial of SOURCE_TRIALS) {
  test(`explicit ${trial.sourceAsset} trial attaches only to a real constructed LOCAL Shallow Mine and stays seat-local`, async ({ page }) => {
    ...
    const result = await page.evaluate(async sourceAsset => {
    ...
    expect(result.sourceAsset).toBe(trial.sourceAsset);
    expect(result.candidateRole).toBe(trial.candidateRole);
    ...
    const plan = await page.evaluate(async () => {
      const helper = await import('./creation-machine-resource-adoption.mjs');
      return helper.inspectCreationMachineResourceSet();
    });
    expect(plan.candidates.map(candidate => candidate.sourceAsset)).toEqual(['shallow-mine-entrance', 'deep-mine-head']);
    expect(plan.candidates[0].candidateRole).toBe('primary-static-candidate');
    expect(plan.candidates[1].candidateRole).toBe('explicit-alternate-static-candidate');

    await page.waitForTimeout(500);
    await page.screenshot({ path: `test-results/${trial.screenshot}`, fullPage: true });

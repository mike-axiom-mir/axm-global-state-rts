...
assert.equal(CREATION_MACHINE_STARTER_SET.length, expectedFixtureIds.length);
assert.deepEqual(CREATION_MACHINE_STARTER_SET.map(entry => entry.fixtureAssetId), expectedFixtureIds);
assert.equal(new Set(CREATION_MACHINE_STARTER_SET.map(entry => entry.sourceAsset)).size, expectedFixtureIds.length);
...
for (const entry of CREATION_MACHINE_STARTER_SET) {
  ...
  assert.equal(creationMachineStarterSetEntry(entry.fixtureAssetId), entry);
...
assert.equal(plan.length, expectedFixtureIds.length);
...
console.log(JSON.stringify({
  ...
  truthBoundary: 'Candidate mapping and fallback policy verified; browser import, visual acceptance, scale, collision, navigation, LOD handoff, FPS and animation remain separate evidence.'
}, null, 2));

...
function progressToLocalDebit(authority, api, controllerKind, accountId, transferId) {
  ...
  assert.equal(authority.globalSalvageCredits.meta().creditCount, 0,
    'LOCAL debit must not itself append global credit');
...
function exerciseMemory(controllerKind, accountId) {
  ...
  assert.equal(finalized.body.globalCreditSummary.spendableMilli, 0,
    'durable credit evidence remains intentionally non-spendable');
  assert.equal(finalized.body.reservation.reservedScrapMilli, 0,
    'committed transfer must consume exactly its matching reservation');
  assert.ok(finalized.body.reservationConsumption.record.applied,
    'reservation consumption must have a durable applied marker');
  ...
  const revisions = {
    ...
    transaction: authority.salvageTransferTransactions.meta().revision,
  ...
  assert.equal(authority.salvageTransferTransactions.meta().revision, revisions.transaction);
  assert.equal(authority.salvageReservationConsumptions.meta().revision, revisions.consumption);

  const obsoleteRelease = call(api, 'POST', '/api/world/local-seat/salvage-release', {
    participantId: progressed.participantId,
    regionSeatId: 'seat-1',
    amountMilli: 1000
  });
  assert.equal(obsoleteRelease.status, 400);
  assert.equal(obsoleteRelease.body.reason, 'verified-local-salvage-reservation-empty',
    'there must be no separately releasable reservation left after a fully covered committed transfer');
  ...
  return Object.freeze({
    ...
    reservationAfterMilli: finalized.body.reservation.reservedScrapMilli
  });
}
...
assert.deepEqual(human, machine,
  'human and machine participants must retain actor-independent LOCAL/economic outcomes under equivalent conditions');
...
try {
  ...
  const consumptionPath = path.join(durableDir, 'reservation-consumption.jsonl');
  const options = () => ({
    worldEpochMs: 0,
    apmCap: 120,
    clock,
  ...
  assert.equal(restarted.globalSalvageCredits.creditForTransfer(transferId).entryHash,
    ...
    'restart must recover already-durable global credit evidence');
  ...
  assert.equal(reconciled.body.reused, true,
    'retry reuses durable global credit, commits the missing transaction event, then reconciles reservation consumption');
  ...
  assert.equal(restarted.verifiedLocalSalvageReservations.summary(progressed.participantId).reservedScrapMilli, 0,
    'restart reconciliation consumes the exact committed reservation once');
  assert.ok(restarted.salvageReservationConsumptions.record(transferId).applied);
  assert.equal(restarted.localSeatStatus({ participantId: progressed.participantId, regionSeatId: 'seat-1' }).journal.revision, 2,
    'global-credit/reservation reconciliation must not advance or re-debit the LOCAL journal');
